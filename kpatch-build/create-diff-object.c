...
/* Default to the x86 architecture. */
enum architecture arch = DEFAULT_ARCH;
...
static int is_gcc6_localentry_bundled_sym(struct symbol *sym)
{
	if (arch == PPC64)
		/*
		 * On ppc64le, the function prologue generated by GCC 6+ has
		 * the sequence:
		 *
		 *	.globl my_func
		 ...
		 *
		 * my_func is the global entry point, which, when called, sets
		 * up the TOC.  .localentry is the local entry point, for calls
		 * to the function from within the object file.  The local
		 * entry point is 8 bytes after the global entry point.
		 */
...
static int kpatch_line_macro_change_only_ppc64le(struct section *sec)
{
...
static struct argp_option options[] = {
	{"arch",	777, "ARCH", 0, "Architecture: x86_64, powerpc [default: x86_64]"},
...
static error_t parse_opt (int key, char *arg, struct argp_state *state)
{
	...
	switch (key)
	{
		...
		case 777: /* --arch */
			...
			else if (!strcmp(arg, "powerpc"))
				arguments->arch = PPC64;
			else
				argp_failure (state, 1, 0, "invalid value for --arch: %s", arg);
			break;
		case ARGP_KEY_ARG:
			if (state->arg_num >= 7)
				/* Too many arguments. */
...
int main(int argc, char *argv[])
{
	...
	if (arguments.klp_arch)
		KLP_ARCH = true;
	if (arguments.arch != DEFAULT_ARCH)
		arch = arguments.arch;
