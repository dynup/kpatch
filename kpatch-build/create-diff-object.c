...
struct section *create_new_text_section(char *name, int entsize, int nr)
{
	struct section *sec;
	int size = entsize * nr;
...
struct section *create_new_rela_section(char *name, struct section *base)
{
	struct section *sec;
...
void *create_section_pair(struct kpatch_elf *kelf, char *name, int entsize,
                         int nr, struct section **sec, struct section **relasec)
{
	char *relaname;
	...
	*sec = create_new_text_section(name, entsize, nr);
	*relasec = create_new_rela_section(name, *sec);

	list_add_tail(&(*sec)->list, &kelf->sections);
	list_add_tail(&(*relasec)->list, &kelf->sections);
                                    ...
                                    char *objname)
{
	...
	funcs = create_section_pair(kelf, ".kpatch.funcs", sizeof(*funcs),
	                            nr, &sec, &relasec);
                                         ...
                                         char *objname)
{
	...
	/* create text/rela section pair */
	dynrelas = create_section_pair(kelf, ".kpatch.dynrelas", sizeof(*dynrelas),
	                               nr, &sec, &relasec);
...
void kpatch_create_mcount_sections(struct kpatch_elf *kelf)
{
	...
	/* create text/rela section pair */
	funcs = create_section_pair(kelf, "__mcount_loc", sizeof(*funcs),
	                            nr, &sec, &relasec);
