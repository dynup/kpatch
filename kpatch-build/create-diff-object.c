...
int kpatch_include_changed_functions(struct kpatch_elf *kelf)
{
	...
	list_for_each_entry(sym, &kelf->symbols, list) {
		if (sym->type == STT_FUNC) {
			...
			else if (!strncmp(sym->name, "kpatch_", 7))
				log_normal("hook function: %s\n", sym->name);
			else
				continue;
			changed_nr++;
...
static int is_sym_hook(struct symbol *sym, char *prefix)
{
	return (sym->type == STT_FUNC && sym->status == NEW &&
	       !strncmp(sym->name, prefix, strlen(prefix))); 
}

void kpatch_create_hook_section(struct kpatch_elf *kelf, char *secname,
                                char *prefix, char *objname)
{
	int nr, size, index, objname_offset;
	struct section *sec, *relasec;
	struct symbol *sym, *strsym;
	struct rela *rela;
	struct kpatch_patch_hook *hooks;
	char *relaname;

	/* count hook functions */
	nr = 0;
	list_for_each_entry(sym, &kelf->symbols, list)
		if (is_sym_hook(sym, prefix))
			nr++;
	...
	if (!sec->data)
		ERROR("malloc");
	sec->data->d_buf = hooks;
	sec->data->d_size = size;
	sec->data->d_type = ELF_T_BYTE;

	/* set section header */
	sec->sh.sh_type = SHT_PROGBITS;
	sec->sh.sh_entsize = sizeof(*hooks);
	sec->sh.sh_addralign = 8;
	sec->sh.sh_flags = SHF_ALLOC;
	sec->sh.sh_size = size;

	/* create hook rela section */

	/* allocate section resources */
	ALLOC_LINK(relasec, &kelf->sections);
	relaname = malloc(strlen(secname) + strlen(".rela") + 1);
	if (!relaname)
		ERROR("malloc");
	strcpy(relaname, ".rela");
	strcat(relaname, secname);
	...
	/* populate hook rela section */
	index = 0;
	list_for_each_entry(sym, &kelf->symbols, list) {
		if (!is_sym_hook(sym, prefix))
			continue;

		/* add rela to fill in hook field */
		ALLOC_LINK(rela, &relasec->relas);
		rela->sym = sym;
		rela->type = R_X86_64_64;
		rela->addend = 0;
		rela->offset = index * sizeof(*hooks) +
			offsetof(struct kpatch_patch_hook, hook);
		...
		rela->addend = objname_offset;
		rela->offset = index * sizeof(*hooks) +
			offsetof(struct kpatch_patch_hook, objname);
	...
	/* sanity check, index should equal nr */
	if (index != nr)
		ERROR("size mismatch in %s section", secname);

}

void kpatch_create_hook_sections(struct kpatch_elf *kelf, char *objname)
{
	kpatch_create_hook_section(kelf, ".kpatch.hooks.load",
	                           "kpatch_load_", objname);
	kpatch_create_hook_section(kelf, ".kpatch.hooks.unload",
	                           "kpatch_unload_", objname);
}
...
int main(int argc, char *argv[])
{
	...
	kpatch_create_hook_sections(kelf_out, name);
